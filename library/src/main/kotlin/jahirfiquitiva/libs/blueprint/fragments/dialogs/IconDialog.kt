...
package jahirfiquitiva.libs.blueprint.fragments.dialogs
            ...
            with(iconView) {
                if (resId > 0) {
                    ...
                    Palette.from(icon).generate(Palette.PaletteAsyncListener { palette ->
                        ...
                        if (activity.usesDarkTheme) {
                            if (color.isColorLight) correctTextColor = color
                            else correctTextColor = activity.accentColor
                        } else {
                            if (color.isColorDark) correctTextColor = color
                            else correctTextColor = activity.accentColor
                        }
